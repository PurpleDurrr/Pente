...
public class yuchenson {
	...
	yuchenHelper purpleBD;
	
	
	public yuchenson(PenteGameBoard b, int stoneColor){
		myBoard = b;
		...
		purpleBD = new yuchenHelper(myBoard, opponentStoneColor);
	...
	public Square doComputerMove(int lastMoveRow, int lastMoveCol){
		...
		purpleBD.assessBoard(lastMoveCol,lastMoveRow);
		
		Square nextMove = null;
		nextMove = purpleBD.blockEveryWang(purpleBD.getPurpleGroup4(),4);
		if(nextMove == null){
		...
		if(nextMove == null) {
			...
			if (nextMove == null){
				...
				if (nextMove == null){
					...
					//if(nextMove== null){
					      //nextMove = this.offense(group2,2);
							if(nextMove == null){
...
/*	public Square offense(ArrayList<OpponentGroup> whatGroup, int whatGroupSize){
		Square nextMove = null;
